...
public class Calculator {
    ...
    public static void startCalculateGame() {
        ...
        Engine.checkResult(CALCULATE_GAME_TASK, calculateGameData);
    }

    private static String[] generateAnswerQuestionPair() {

        int randomOperation = RANDOMIZER.nextInt(MATH_OPERATIONS.length);

        String randomMathSign = MATH_OPERATIONS[randomOperation];

        int randomNumberOne = RANDOMIZER.nextInt(BOUND_OF_RANDOM_VALUES);
        int randomNumberTwo = RANDOMIZER.nextInt(BOUND_OF_RANDOM_VALUES);
